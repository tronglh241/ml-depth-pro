...
class DepthProEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        x0_encodings = self.patch_encoder(x0_patches)
        ...
        # )
        x_latent0_features = self.reshape_feature(
            self.backbone_highres_hook0,
            out_size * 4,
            out_size * 4,
        )
        ...
        # )
        x_latent1_features = self.reshape_feature(
            self.backbone_highres_hook1,
            out_size * 4,
            out_size * 4,
        )

        x1_encodings = self.patch_encoder(x1_patches)
        x2_encodings = self.patch_encoder(x2_patches)
        ...
        x0_features = self.reshape_feature(
            x0_encodings,
            out_size * 4,
            out_size * 4,
        )
        ...
        x1_features = self.reshape_feature(
            x1_encodings,
            out_size * 2,
            out_size * 2,
        )
        ...
        # 24x24 feature maps.
        x2_features = self.reshape_feature(
            x2_encodings,
            out_size,
            out_size,
        )
        ...
        x_global_features = self.reshape_feature(
            x_global_features, out_size, out_size
        )
